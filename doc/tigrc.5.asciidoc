...
These actions start with a '!' followed by zero or more option flags and finally
the command that should be executed. By default, commands are run in the
foreground with their console output shown. The following option flags allow
...
Option flags can be combined, e.g. "!?<git commit" will prompt the user whether
