...
'refresh-mode' (mixed) [manuel|auto|periodic|<bool>]::

	Configures how views are refreshed based on modifications to watched
	files in the repository. When set to 'manuel', nothing is refreshed
	automatically. When set to 'auto', views are refreshed when a
	modification is detected. When set to 'periodic', visible views are
	refreshed periodically using 'refresh-interval'.
...
the 'display' setting. When no 'display' setting value is given, 'yes' is
assumed. For 'display' settings taking an enumerated value this will
automatically resolve to the default enum value. For example, `file-name` will
...
Note that all column options can be toggled. For 'display' options, use the
