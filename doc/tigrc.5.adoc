...
'refresh-mode' (mixed) [manual|auto|after-command|periodic|entr|<bool>]::
	...
	http://entrproject.org/[entr] reports a relevant filesystem event.
	The `entr` executable must be installed separately from tig and
	visible on `$PATH`.
